...
/**
 * This class modifies the TimSort class from the Android Open Source Project,
 * mainly by adding the TimeSortDataFormat interface to control access
...
public final class TimSort<K, ARRAY> {
     ...
     */
    private ARRAY tmp; // Actual runtime type will be Object[], regardless of T
    /**
    ...
    private TimSort(int len, Comparator<? super K> c, TimSortDataFormat<K, ARRAY> format) {
        ...
        // int len = format.getLength(a);
//        @SuppressWarnings({"unchecked", "UnnecessaryLocalVariable"})
//        T[] newArray = (T[]) new Object[len < 2 * INITIAL_TMP_STORAGE_LENGTH ?
//                len >>> 1 : INITIAL_TMP_STORAGE_LENGTH];
//        tmp = newArray;
        tmp = format.createArray(len < 2 * INITIAL_TMP_STORAGE_LENGTH ?
                len >>> 1 : INITIAL_TMP_STORAGE_LENGTH);
        /*
    ...
    public void clear() {
        Arrays.fill(runBase, 0);
        Arrays.fill(runLen, 0);
        format.clearArray(tmp);
        this.stackSize = 0;
    ...
    private void sort(ARRAY a, int lo, int hi) {
        ...
        if (DEBUG) assert nRemaining <= this.fixedLength;
        int minRun = minRunLength(nRemaining);
        do {
            // Identify next run
            int runLen = countRunAndMakeAscending(a, lo, hi, c, format);
            // If run is short, extend to min(minRun, nRemaining)
            if (runLen < minRun) {
                int force = nRemaining <= minRun ? nRemaining : minRun;
                binarySort(a, lo, lo + force, lo + runLen, c, format);
                runLen = force;
            }
            // Push run onto pending-run stack, and maybe merge
            this.pushRun(lo, runLen);
            this.mergeCollapse(a);
    ...
    private static <K, ARRAY> void binarySort(ARRAY a, int lo, int hi, int start, Comparator<? super K> c, TimSortDataFormat<K, ARRAY> format) {
        ...
        for ( ; start < hi; start++) {
            // T pivot = a[start];
            final K pivot = format.get(a, start);
            format.copyTo(a, start, pivotCache, 0);
            ...
            while (left < right) {
                ...
                if (c.compare(pivot, format.get(a, mid)) < 0)
                    right = mid;
                else
                    left = mid + 1;
            }
            ...
            switch(n) {
                ...
                case 2:  format.copyTo(a, left + 1, a, left + 2);
                case 1:  format.copyTo(a, left, a, left + 1);
                    break;
                // default: System.arraycopy(a, left, a, left + 1, n);
                default: format.arrayCopy(a, left, a, left + 1, n);
            }
            //a[left] = pivot;
            format.copyTo(pivotCache, 0, a, left);
        }
    }
    /**
    ...
    private static <K, ARRAY> int countRunAndMakeAscending(ARRAY a, int lo, int hi, Comparator<? super K> c, TimSortDataFormat<K, ARRAY> format) {
        ...
        if (c.compare(format.get(a, runHi++), format.get(a, lo)) < 0) { // Descending
            // while(runHi < hi && c.compare(a[runHi], a[runHi - 1]) < 0)
            while(runHi < hi && c.compare(format.get(a, runHi), format.get(a, runHi - 1)) < 0)
                runHi++;
            reverseRange(a, lo, runHi, format);
        } else {                              // Ascending
            // while (runHi < hi && c.compare(a[runHi], a[runHi - 1]) >= 0)
            while (runHi < hi && c.compare(format.get(a, runHi), format.get(a, runHi - 1)) >= 0)
                runHi++;
...
//            a[hi--] = t;
     ...
     * entry to the method.
     */
    private void mergeCollapse(ARRAY a) {
        while (stackSize > 1) {
            int n = stackSize - 2;
     ...
     */
    private void mergeForceCollapse(ARRAY a) {
        while (stackSize > 1) {
            int n = stackSize - 2;
     ...
     */
    private void mergeAt(ARRAY a, int i) {
        ...
        int k = gallopRight(format.get(a, base2), a, base1, len1, 0, c, format);
        if (DEBUG) assert k >= 0;
        base1 += k;
        len1 -= k;
        if (len1 == 0)
        ...
        len2 = gallopLeft(format.get(a, base1 + len1 - 1), a, base2, len2, len2 - 1, c, format);
     ...
     * @param dataFormat array data formatter
    ...
    private static <K, ARRAY> int gallopLeft(K key, ARRAY a, int base, int len,
        ...
        if (c.compare(key, dataFormat.get(a, base + hint)) > 0) {
            // Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
            int maxOfs = len - hint;
            // while (ofs < maxOfs && c.compare(key, a[base + hint + ofs]) > 0) {
            while (ofs < maxOfs && c.compare(key, dataFormat.get(a, base + hint + ofs)) > 0) {
                lastOfs = ofs;
                ofs = (ofs * 2) + 1;
        ...
        while (lastOfs < ofs) {
            ...
            if (c.compare(key, dataFormat.get(a ,base + m)) > 0)
    ...
    private static <K, ARRAY> int gallopRight(K key, ARRAY a, int base, int len,
        ...
        int lastOfs = 0;
        // if (c.compare(key, a[base + hint]) < 0) {
        if (c.compare(key, format.get(a, base + hint)) < 0) {
            // Gallop left until a[b+hint - ofs] <= key < a[b+hint - lastOfs]
            int maxOfs = hint + 1;
            // while (ofs < maxOfs && c.compare(key, a[base + hint - ofs]) < 0) {
            while (ofs < maxOfs && c.compare(key, format.get(a, base + hint - ofs)) < 0) {
                lastOfs = ofs;
                ofs = (ofs * 2) + 1;
                if (ofs <= 0)   // int overflow
                    ofs = maxOfs;
        ...
        while (lastOfs < ofs) {
            int m = lastOfs + ((ofs - lastOfs) >>> 1);
            // if (c.compare(key, a[base + m]) < 0)
            if (c.compare(key, format.get(a,base + m)) < 0)
                ofs = m;          // key < a[b + m]
            else
     ...
     */
    private void mergeLo(ARRAY a, int base1, int len1, int base2, int len2) {
        if (DEBUG) assert len1 > 0 && len2 > 0 && base1 + len1 == base2;
        // Copy first run into temp array
        // ARRAY a = this.a; // For performance
        ARRAY tmp = ensureCapacity(len1, format);
        // System.arraycopy(a, base1, tmp, 0, len1);
        format.arrayCopy(a, base1, tmp, 0, len1);
        int cursor1 = 0;       // Indexes into tmp array
        int cursor2 = base2;   // Indexes int a
        int dest = base1;      // Indexes int a
        // Move first element of second run and deal with degenerate cases
        // a[dest++] = a[cursor2++];
        format.copyTo(a, cursor2++, a, dest++);
        if (--len2 == 0) {
            // System.arraycopy(tmp, cursor1, a, dest, len1);
            format.arrayCopy(tmp, cursor1, a, dest, len1);
            return;
        }
        if (len1 == 1) {
            // System.arraycopy(a, cursor2, a, dest, len2);
            format.arrayCopy(a, cursor2, a, dest, len2);
            // a[dest + len2] = tmp[cursor1]; // Last elt of run 1 to end of merge
            format.copyTo(tmp, cursor1, a ,dest + len2);
            return;
        }
        ...
        while (true) {
            ...
            do {
                ...
                if (c.compare(format.get(a, cursor2), format.get(tmp, cursor1)) < 0) {
                    // a[dest++] = a[cursor2++];
                    format.copyTo(a, cursor2++, a, dest++);
                    count2++;
                    count1 = 0;
                    ...
                    // a[dest++] = tmp[cursor1++];
                    format.copyTo(tmp, cursor1++, a, dest++);
                    count1++;
                    count2 = 0;
                    if (--len1 == 1)
                        break outer;
            ...
            do {
                ...
                count1 = gallopRight(format.get(a, cursor2), tmp, cursor1, len1, 0, c, format);
                if (count1 != 0) {
                    // System.arraycopy(tmp, cursor1, a, dest, count1);
                    format.arrayCopy(tmp, cursor1, a, dest, count1);
                    dest += count1;
                    cursor1 += count1;
                    len1 -= count1;
                ...
                count2 = gallopLeft(format.get(tmp, cursor1), a, cursor2, len2, 0, c, format);
                if (count2 != 0) {
                    // System.arraycopy(a, cursor2, a, dest, count2);
                    format.arrayCopy(a, cursor2, a, dest, count2);
                    dest += count2;
                    cursor2 += count2;
                    len2 -= count2;
                    if (len2 == 0)
     ...
     * @param len2  length of second run to be merged (must be > 0)
     */
    private void mergeHi(ARRAY a, int base1, int len1, int base2, int len2) {
        if (DEBUG) assert len1 > 0 && len2 > 0 && base1 + len1 == base2;
        // Copy second run into temp array
        // ARRAY a = this.a; // For performance
        ARRAY tmp = ensureCapacity(len2, format);
        //System.arraycopy(a, base2, tmp, 0, len2);
        format.arrayCopy(a, base2, tmp, 0, len2);
        int cursor1 = base1 + len1 - 1;  // Indexes into a
        int cursor2 = len2 - 1;          // Indexes into tmp array
        int dest = base2 + len2 - 1;     // Indexes into a
        // Move last element of first run and deal with degenerate cases
        // a[dest--] = a[cursor1--];
        format.copyTo(a, cursor1--, a, dest--);
        if (--len1 == 0) {
            // System.arraycopy(tmp, 0, a, dest - (len2 - 1), len2);
            format.arrayCopy(tmp, 0, a, dest - (len2 - 1), len2);
            return;
        }
        if (len2 == 1) {
            dest -= len1;
            ...
            format.arrayCopy(a, cursor1 + 1, a, dest + 1, len1);
            // a[dest] = tmp[cursor2];
            format.copyTo(tmp, cursor2, a, dest);
            return;
        }
        // Comparator<? super T> c = this.c;  // Use local variable for performance
        int minGallop = this.minGallop;    //  "    "       "     "      "
        ...
        while (true) {
            ...
            do {
                ...
                if (c.compare(format.get(tmp, cursor2), format.get(a, cursor1)) < 0) {
                    // a[dest--] = a[cursor1--];
                    format.copyTo(a, cursor1--, a, dest--);
                    count1++;
                    count2 = 0;
            ...
            do {
                ...
                count1 = len1 - gallopRight(format.get(tmp,cursor2), a, base1, len1, len1 - 1, c, format);
                if (count1 != 0) {
                    dest -= count1;
                    cursor1 -= count1;
                    len1 -= count1;
                    //System.arraycopy(a, cursor1 + 1, a, dest + 1, count1);
                    format.arrayCopy(a, cursor1 + 1, a, dest + 1, count1);
                    if (len1 == 0)
                        break outer;
                }
                //a[dest--] = tmp[cursor2--];
                ...
                count2 = len2 - gallopLeft(format.get(a, cursor1), tmp, 0, len2, len2 - 1, c, format);
                if (count2 != 0) {
                    dest -= count2;
                    cursor2 -= count2;
                    len2 -= count2;
                    //System.arraycopy(tmp, cursor2 + 1, a, dest + 1, count2);
                    format.arrayCopy(tmp, cursor2 + 1, a, dest + 1, count2);
    ...
    private ARRAY ensureCapacity(int minCapacity, TimSortDataFormat<K, ARRAY> format) {
        if (format.getLength(tmp) < minCapacity) {
            ...
            if (newSize < 0) // Not bloody likely!
                ...
                newSize = Math.min(newSize, fixedLength >>> 1);
            // @SuppressWarnings({"unchecked", "UnnecessaryLocalVariable"})
            ARRAY newArray = format.createArray(newSize);
            tmp = newArray;
        }
        return tmp;
    }
