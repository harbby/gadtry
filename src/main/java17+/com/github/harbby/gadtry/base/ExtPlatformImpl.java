        ...
        implements Platform.ExtPlatform
{
    ...
    public <A> Comparator<A> getArrayComparator(Class<A> typeClass)
    {
        try {
            MethodHandles.Lookup lookup = MethodHandles.lookup();
            MethodType methodType = MethodType.methodType(int.class, typeClass, typeClass);
            MethodHandle methodHandle = lookup.findStatic(Arrays.class, "compare", methodType);
            return (Comparator<A>) LambdaMetafactory.metafactory(
                    lookup, "compare",
            ...
            throw new PlatFormUnsupportedOperation("this platform not find Arrays.compare()", e);
        }
    }
